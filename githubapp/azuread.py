...
class AzureAD:
    ...
    def get_group_members(self, token=None, group_name=None):
        ...
        try:
            ...
            members = self.get_group_members_pages(token, f'{self.AZURE_API_ENDPOINT}/groups/{group_info["id"]}/members')
    ...
    def get_group_members_pages(self, token=None, url=None):
        ...
        if members_data.ok != True:
          print(f"[GetMembers]: Error getting members data error code {members_data.status_code}")
          return []
        
        members_data_content = members_data.json()
        members = members_data_content["value"]
        if "@odata.nextLink" in members_data_content:     
          members.extend(self.get_group_members_pages(token, members_data_content["@odata.nextLink"]))   
        return members          
